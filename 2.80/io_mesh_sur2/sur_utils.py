...
def _is_ascii_file(data):
    ...
    if size == 0:  # Odds to get that result from an ASCII file are null...
        print("WARNING! Reported size (facet number) is 0, assuming invalid binary SUR file.")
...
def write_sur2(filepath="",
              faces=(),
              ascii=False,
              ):
    """
    Write a sur file from faces,
...
def read_sur2(filepath):
    """
    Return the triangles and points of an sur binary file.

    Please note that this process can take lot of time if the file is
    huge (~1m30 for a 1 Go sur file on an quad core i7).
    ...
    Example of use:
       ...
       >>> tris, tri_nors, pts = read_sur(filepath)
       >>> pts = list(pts)
       >>>
...
def read_sur(filepath):
    ...
    return tris, norms, verts



def write_sur(filepath, faces, verts=[]):
    # the SUR file format is :
    # numVertices
    # x y z
    # x y z
